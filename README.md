> Last updated: 2026-09-16 17:40 KST
...
---
# Isaac Sim Warehouse 적재 시나리오

이 저장소에는 Worker가 Cart를 밀고, 지정된 박스를 순차적으로 Cart 위에 적재한 뒤, 적재된 박스와 Cart를 함께 NavMesh 안에서 이동시키는 실험 코드가 포함되어 있습니다. 실행 중에는 원본 USD를 저장하지 않고 runtime session layer에 변환을 적용하며, Stop 시 runtime 변환을 정리합니다.

### 실행 순서

1. Isaac Sim에서 `warehouse_cart_worker.usd`를 열고 Timeline을 Stop 상태로 둡니다.
2. Script Editor에서 `scripts/run_worker_stack_gesture.py`를 열어 Run합니다.
3. `READY` 및 `AFTER_STACK_ENABLED` 메시지를 확인합니다.
4. Isaac Sim의 Play 버튼을 눌러 Worker·Cart 이동과 박스 적재를 시작합니다.
5. 세 박스 적재가 끝나면 Cart와 적재 박스의 NavMesh 주행을 확인합니다.
6. Stop을 눌러 runtime 변환을 해제합니다.

### 최종 시나리오에 사용되는 파일

| 파일 | 역할 |
|---|---|
| `scripts/run_worker_stack_gesture.py` | 위 시나리오의 유일한 직접 실행 launcher |
| `scripts/setup_warehouse_stack_gesture.py` | Worker·Cart·박스 적재 runtime 초기화 |
| `scripts/setup_warehouse_runtime.py` | NavMesh 확인, Worker Behavior, Cart 동기화 준비 |
| `scripts/warehouse_worker_cart_pose_sync.py` | Worker 이동에 맞춰 Cart의 위치·회전 동기화 |
| `scripts/warehouse_worker_face_box.py` | Worker의 박스 방향 회전과 도착 이벤트 처리 |
| `scripts/warehouse_box_transfer.py` | 박스를 원래 위치에서 Cart 위 목표 위치로 보간 이동 |
| `scripts/warehouse_box_only_stack.py` | 박스 순서와 적재 높이 계산 |
| `scripts/warehouse_stack_gesture.py` | 박스별 이동·적재 상태를 관리하는 상태 머신 |
| `scripts/warehouse_worker_reach_gesture.py` | 박스 높이를 따라가는 시각적 팔 동작 |
| `scripts/warehouse_multi_box_config.py` | 박스 Prim 경로, 적재 순서, 팔 동작 시간 설정 |
| `scripts/warehouse_loaded_cart_roaming.py` | 적재 완료 후 Cart·Worker 자유 주행 상태 머신 |
| `scripts/warehouse_roaming_actions.py` | NavMesh 목적지 검증과 Cart 이동 명령 |
| `scripts/warehouse_roaming_goals.py` | 연결된 NavMesh 내 새 목적지 후보 생성 |
| `scripts/warehouse_roaming_cargo.py` | Cart 이동 시 적재 박스의 상대 위치 유지 |
| `scripts/warehouse_roaming_config.py` | 자유 주행 활성화 및 목적지 탐색 파라미터 |

팔 동작과 박스 이동은 현재 물리적 grasp가 아닌 시각적 연출입니다. 따라서 VLM이 Worker의 팔 동작, 박스 위치 변화, Cart 적재 상태를 관찰해 “운반 중” 상황으로 해석하도록 하는 실험에 적합합니다.
